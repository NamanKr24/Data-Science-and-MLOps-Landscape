...
# Cloud usage by Industry
      ...
      data5 <- bind_cols(data_long2, data_long)
      ...
      data5 <- data5 %>%
        filter(Selected %in% c("Amazon Web Services (AWS)", "Google Cloud Platform (GCP)", "Microsoft Azure"))
      ...
      data5 <- data5 %>%
        mutate(Selected = ifelse(Selected == "Amazon Web Services (AWS)", "AWS", Selected)) %>%
        mutate(Selected = ifelse(Selected == "Google Cloud Platform (GCP)", "GCP", Selected)) %>%
        mutate(Selected = ifelse(Selected == "Microsoft Azure", "Azure", Selected))
...
# ML experience by Programming experience
      ...
      data6$Q16 <- factor(data6$Q16, levels = c("20+ years", "10-20 years", "5-10 years", "4-5 years",
                                                "3-4 years", "2-3 years", "1-2 years", "< 1 year"))
      
      data6$Q11 <- factor(data6$Q11, levels = c("< 1 years", "1-3 years",
...
# ML state by roles
      ...
      data8 <- bind_cols(data_long4, data_long3)
      ...
      node_labels <- unique(c(data8$Q23, data8$Selected))
      
      data8$source_index <- match(data8$Q23, node_labels) - 1
      
      data8$target_index <- match(data8$Selected, node_labels) - 1